...
class ESP32Connection:
    ...
    async def send_avatar_set_fetch(
        ...
        if self._active_avatar_checksum == checksum:
            return {
                "ok": True,
                "checksum": checksum,
                "already_loaded": True,
            }
        previous = self._avatar_set_waiters.get(checksum)
        if previous is not None and not previous.done():
            try:
                return await asyncio.wait_for(
                    asyncio.shield(previous), timeout=timeout
                )
            except asyncio.TimeoutError:
    ...
    def handle_avatar_set_loaded(self, payload: dict[str, Any]) -> None:
        ...
        if payload.get("ok") is True:
            self._active_avatar_checksum = checksum
...
class ESP32Manager:
            ...
            async for message in ws:
                ...
                if msg_type == "hello":
                    ...
                    task.add_done_callback(
                        lambda t: self._init_tasks.remove(t)
    ...
    async def _emit_stackchan_event(self, payload: dict[str, Any]) -> None:
        ...
        if event_type == "behavior":
            ...
            if not isinstance(behavior_id, str) or not behavior_id:
                logger.warning(
                    "Malformed behavior event: behavior_id=%r",
                    behavior_id,
                )
                return
            waiter = self._behavior_waiters.pop(
                (session_id, behavior_id), None
            )
            if waiter is None:
                logger.warning(
                    "Unmatched behavior event: %s id=%s session=%s",
                    subtype,
                    behavior_id,
                    session_id,
                )
        ...
        if event_type != "touch":
            logger.warning(
                "Malformed stackchan-event frame: event_type=%r",
                event_type,
            )
            return
    ...
    async def _perform_xc_body_behavior(
        ...
        *,
        ...
        if not connection.initialized:
            return None, {
                "code": -32000,
                "message": "ESP32 not initialized",
            }

        key = (connection.session_id, behavior_id)
        async with self._tool_lane_locks["servo"]:
    ...
    def _fail_behavior_waiters(self, session_id: str) -> None:
        for key, waiter in list(self._behavior_waiters.items()):
            ...
            self._behavior_waiters.pop(key, None)
            if not waiter.done():
                waiter.set_exception(ConnectionError("ESP32 disconnected"))
