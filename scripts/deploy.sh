...
RUNTIME_TAG=$REGISTRY_REPO:stackchan-mcp-0.17.0
...
copy_runtime_inputs() {
  local context=$1 upstream_root=$2 file_list=$3

  if [ "$candidate" = "1" ]; then
    git -C "$REPO" ls-files -z --cached --others --exclude-standard -- \
      gateway stackchan contracts deploy scripts > "$file_list"
    COPYFILE_DISABLE=1 tar -cf - -C "$REPO" --null -T "$file_list" \
      | tar -xf - -C "$context/app"
  else
    git -C "$REPO" archive HEAD \
      gateway stackchan contracts deploy scripts \
      | tar -xf - -C "$context/app"
  fi
  ...
  git -C "$REPO/references/stackchan-mcp" archive HEAD \
    | tar -xf - -C "$upstream_root"
  git -C "$upstream_root" apply \
    "$context/app/stackchan/stackchan-mcp-native-avatar.patch"
  mv "$upstream_root/gateway" "$context/stackchan-gateway"
}

prepare_context() {
  local context=$1 upstream_root=$2 file_list=$3
  mkdir -p "$context/app" "$context/avatar" "$upstream_root"
  copy_runtime_inputs "$context" "$upstream_root" "$file_list"
  python3 "$context/app/scripts/build_avatar_assets.py" \
    --output-dir "$context/avatar" >/dev/null
}

build_and_push_runtime() {
  local context=$1 source_commit=$2 upstream_commit=$3 dirty=$4 built_at=$5
...
deploy_images() {
  local runtime_ref=$1 caddy_ref=$2 source_commit=$3 upstream_commit=$4
  local avatar_sha256=$5 deployment_kind=$6
  login_vm_to_registry
  ssh_vm bash -s -- \
    "$runtime_ref" "$caddy_ref" "$source_commit" "$upstream_commit" \
    "$avatar_sha256" "$deployment_kind" < "$REPO/deploy/install.sh"
  configure_openclaw
  probe_openclaw
}
...
upstream_commit=$(
  git -C "$REPO" ls-tree "$source_commit" references/stackchan-mcp \
    | awk '{print $3}'
)
actual_upstream=$(git -C "$REPO/references/stackchan-mcp" rev-parse HEAD)
[ "$upstream_commit" = "$actual_upstream" ] \
  || die "StackChan checkout does not match the pinned gitlink" 65
...
upstream_root=$build_root/upstream
file_list=$build_root/files
prepare_context "$context" "$upstream_root" "$file_list"
...
build_and_push_runtime \
  "$context" "$source_commit" "$upstream_commit" "$dirty" "$built_at"
mirror_caddy
